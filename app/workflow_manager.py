...
def _init_worker(config_manager: ConfigManager, project_root: str):
    ...
    input_manager = InputManager(
        ...
        stage_config=config_manager._manager_output_config,
        project_root=project_root
        )
    
    preprocessing_manager = PreprocessingManager(
        config_manager.get_preprocessing_config(),
        stage_config=config_manager._manager_output_config(),
        project_root
    )
    ...
    local_builder = ProcessingBuilder(
        ...
        output_flags=config_manager._get_enabled_outputs()
    )
...
def process_image_worker(paths_config: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        return processing_builder_instance.process_single_image(paths_config)
    except Exception as e:
        logger.error(f"Error en worker (PID: {os.getpid()}) procesando {paths_config}: {e}", exc_info=True)
        return {"error": str(e), "image": paths_config}
...
class WorkflowManager:
    ...
    def run(self, force_mode: Optional[str] = None, workers_override: Optional[int] = None) -> Dict[str, Any]:
        ...
        output_folder = paths_config.get('output_folder', "")
        valid_extensions = tuple(self.config_manager._image_loader_config.get('valid_image_extensions', []))
        ...
        return self._process_images(image_paths, force_mode, workers_override)

    def _process_images(self, image_paths: List[str], force_mode: Optional[str] = None, workers_override: Optional[int] = None) -> Dict[str, Any]:
    ...
    def _process_interactive_mode(self, image_paths: List[str]) -> Dict[str, Any]:
        ...
        for i, image_path in enumerate(image_paths, 1):
            ...
            try:
                result = self.builder._process_single_image(image_paths)
                doc_id = os.path.splitext(os.path.basename(image_paths))[0]
    ...
    def _process_batch_mode(self, image_paths: List[str], workers_override: Optional[int] = None) -> Dict[str, Any]:
        ...
        with ProcessPoolExecutor(
            ...
            initargs=(self.config_manager, self.project_root)
        ) as executor:
