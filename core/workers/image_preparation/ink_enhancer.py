...
from core.utils.image_utils import extract_contours_metrics, make_contiguous, vec_contours_metrics
...
class InkCorrector(ImagePrepAbstractWorker):
    ...
    def delete_outliersvec(self, full_img: np.ndarray[Any, Any]):
        grey_img = make_contiguous(full_img)
        cont_coords_list, metrics = vec_contours_metrics(grey_img)
        ...
        # Unimos las condiciones en un único set O(1) de chequeo rápido
        group_outliers = dist_indices | irreg_indices | rect1ind | rect2ind | vertical_indices | lines_indices | solidity_indices | out_index
        try:
            for idx, cont_coords in cont_coords_list:
                # if idx in out_index:
                #     cv2.drawContours(grey_img, [cont_coords[idx]], -1, self.white, thickness=cv2.FILLED)
