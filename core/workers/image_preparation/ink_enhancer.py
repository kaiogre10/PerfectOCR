...
class InkCorrector(ImagePrepAbstractWorker):
    ...
    def __init__(self, config: Dict[str, Any], project_root: str):
        ...
        self.kernel_threshold = self.worker_config["kernel_threshold"]
    ...
    def process(self, context: Dict[str, Any], manager: DataFormatter) -> bool:
        ...
        try:
            ...
            # kernel = cv2.getStructuringElement(shape=cv2.MORPH_CROSS, ksize=self.kernel_threshold)
            gray_img = self._decolorate(full_img)
            metrics, full_bin_img = extract_cc_metrics(gray_img.copy(), worker_config={}, binarice=True)
            correct = self._restore_faded_ink(gray_img, full_bin_img.copy(), metrics)
    ...
    def _restore_faded_ink(self, gray_img: np.ndarray[Any, Any], full_bin_img: np.ndarray[Any, np.dtype[np.uint8]], metrics: Dict[str, Any]) -> np.ndarray[Any, Any]:
        """
