...
class Binarizator(PreprossesingAbstractWorker):
    ...
    def _process_individual_polygons(self, gray_img: np.ndarray[Any, Any], height: float) -> np.ndarray[np.uint8, Any]:
    ...
    def preprocess(self, cropped_img: CroppedImage, manager: DataFormatter) -> CroppedImage:
        """Binariza la imagen del polígono recibido y retorna la versión binarizada."""
        try:
            # Obtener la imagen numpy del CroppedImage
            gray_img = cropped_img.cropped_img
            # Procesar la imagen individual
            height = gray_img.shape[0]
            bin_img: np.ndarray[np.uint8, Any] = self._process_individual_polygons(gray_img, height)
                        
        except Exception as e:
            logger.error(f"Error en binarización del polígono {cropped_img.polygon_id}: {e}")
            # En caso de error, retornar la imagen original sin procesar
            return cropped_img