...
class DensityScanner(VectorizationAbstractWorker):
    ...
    def vectorize(self, context: Dict[str, Any], manager: DataFormatter) -> bool:
        ...
        try:
            ...
            if table_line_ids:
                ...
                expanded_line_ids: List[str] = self._expand_to_consecutive_interval_by_ids(table_line_ids, list(valid_analyses.keys()))
                total_time = time.time() - start_time
                
                logger.info(f"Detección de tablas completada en {total_time:.4f}s. Encontradas {len(expanded_line_ids)} líneas tabulares")
                ...
                total_time = time.time() - start_time
                logger.error(f"DBSCAN no detectó tablas en el documento. Tiempo: {total_time:.4f}s")
                return False
    ...
    def _expand_to_consecutive_interval_by_ids(self, table_line_ids: List[str], all_line_ids: List[str]) -> List[str]:
        ...
        indices = self._get_consecutive_indices(table_line_ids, all_line_ids)
        if not indices:
    ...
    # def _apply_dbscan_clustering_legacy(self, valid_analyses: List[Dict[str, Dict[str, float]]], valid_indices: List[int]) -> List[int]:
    #     """Aplica DBSCAN para agrupar líneas similares - versión original."""
