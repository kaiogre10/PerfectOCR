...
def test_load_native_library(PROJECT_ROOT: str):
    ...
    container_bin_path = os.path.join(PROJECT_ROOT, "bin", "containers" + binary_extension)
    storage_bin_path = os.path.join(PROJECT_ROOT, "bin", "buffer_handler" + binary_extension)
    ...
    logger.info(f"-> Bytes normales: {len(raw_payload)} Bytes utf-16 mapeados: {len_bytes}")

    # try:
    #     lib.storage_batch_flat(plaintext_c, arreglo_tamanos_c, ctypes.c_size_t(total_elements))
    #     logger.info(f"INYECCIÓN COMPLETADA CON ÉXITO, tiempo: {time.perf_counter() - t0:.8f}'s")
    # except Exception as e:
